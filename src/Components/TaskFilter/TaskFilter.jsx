...
export default function TaskFilter({ val, typeId, onClicked, }) {
  ...
  const buttonSetElement = (btn, index) => {
    if (btn === 'Все') {btnElem =  a.map(abc =>abc) }
    // eslint-disable-next-line no-unused-expressions
    else if (btn === 'Aктивные') {btnElem =  a.filter(abc =>  !abc.isCompleted ) }
    // eslint-disable-next-line no-unused-expressions
    else {btnElem =  a.filter(abc =>  abc.isCompleted  ) }
  ...
  const buttons = btnsFotter.map((btn, ind) => {
          return (
            <li key={ind}>
              <button onClick={() => buttonSetElement(btn, ind)} id={ind} className={elemActive === ind ? 'selected' : ''}>{btn}</button>
              </li>
          )
        })
