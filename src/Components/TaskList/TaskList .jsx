...
export default function TaskList( { newTask, allTasks, donedTasks, newTaskAdd, tasksOf, isBtnTk } ) {
    ...
    const elements = items.map((item) => {
        ...
        return (
            <li key={id}  className={`list-group-item ${useItem}`}>
                <Task label={name} doneTaski={doneTask} id={id} onDeleted={() => { deleteElement(id) }} isCompleted={isCompleted} isDiscription={isDiscription} newTaskOutput={newTask} newTaskInput={newTaskIn} Created={Created} isBtnTk={isBtnTk}
  ...
  return (
      <ul className="list-group todo-list">
          {isLoading ? [...new Array(4)].map((_, ind) => {
